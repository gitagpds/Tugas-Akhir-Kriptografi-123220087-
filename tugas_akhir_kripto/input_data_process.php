...
function aesEncrypt($data, $key) {
    $method = 'aes-256-cbc';
    $key = hash('sha256', $key, true); // Hash the key
...
// Function to embed message into image using LSB encoding, now includes key_for_picture
function lsbEncrypt($imagePath, $message, $key_for_picture) {
    // Encrypt the message using the key_for_picture before hiding it in the image
    $encryptedMessage = aesEncrypt($message, $key_for_picture);  // Use AES encryption with key_for_picture
    $binaryMessage = toBin($encryptedMessage);  // Convert the encrypted message to binary
    ...
    for ($y = 0; $y < $height; $y++) {
        for ($x = 0; $x < $width; $x++) {
            ...
            // Modify the LSB of each color channel to encode the message
            if ($messageIndex < $messageLength) {
            ...
            // Recalculate the new color with the modified LSB
...
$encoded_image_path = lsbEncrypt($image_path, $deskripsi_koleksi, $key_for_picture); // Encoding the message in the image
...
if ($result) {
    header("Location: input_data.php?pesan=input_data_sukses");
} else {
